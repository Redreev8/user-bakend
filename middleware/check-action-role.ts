...
const checkTokenAction = (payload: User | string[] | string | null, url: string) => {
    if (!payload) return
...
const checkActionRole = async (
    ...
    if (checkTokenAction(payloaAuth as string , request) || checkTokenAction(payload as string, request)) {
        next()
        return 
    }
